...
- **Receipt hook lifecycle** — `start` keeps a hand-merged pre-push that carries the check at the current policy, accepts a custom `core.hooksPath` directory once its pre-push carries it (never writing into it), and refuses a hook that declares another policy or is not executable, naming the remedy; before freezing a candidate it runs the kept hook twice, from the worktree root, with a push of a dangling child of HEAD on the current branch — once with no receipt, which the hook must refuse, and once while a temporary receipt names that commit, which the hook must accept — so a marked hook that consults no receipts is refused whichever way it exits; a probe directory left by an interrupted run is swept before the next one. Receipts completed under policy 1 no longer authorise a push: the hook and the adapter require the current policy. Deleted test files are listed for reviewers without counting as regressions. The hook peels annotated tags before the receipt lookup. Each reviewer's open disposition needs its own resolution. The Bash adapter matches hook-disarming options case-insensitively.
