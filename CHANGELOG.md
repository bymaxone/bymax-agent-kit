...
- **A custom hooks directory is no longer told to delete its hook** — every refusal about a hook offered its own remedy, worded differently each time. `start` writes only into the repository's own hooks directory and never into a `core.hooksPath` one, so for that reader the remedy was a way to end up with no check at all. One function answers which case the message is in, and every refusal about a hook ends with its answer.
