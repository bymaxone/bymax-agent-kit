...
- **`bymax-report` — the weekly standup, written from evidence** — `/bymax-report:standup` collects one repository over one period and writes the two sections a silent standup carries: PROGRESS, the asks of the period as outcome sentences with the undelivered ones marked in progress, and UPDATES, what shipped grouped by the product area the reader knows. The asks are read from what a person typed into Claude Code and Codex sessions on that repository — worktree sessions live in sibling directories and are read; tool results, task notifications, harness-injected meta lines, compact summaries, peer messages, Codex `exec` sessions started by the review plugin and Codex subagents are refused, each by a test naming its real shape. The updates are read from `gh pr list` and `git log --all`; a missing `gh` is recorded in coverage and never hidden. Measured on one repository's previous week: 21 PRs, 150 commits and 161 typed requests across three session directories, in a 158 KB file of one record per line. Two things the collect will not do: use `git log --since`, which is a traversal cutoff rather than a filter and hid a parent commit behind a backdated child in the fixture; and write a report line no record supports — an empty request list means no PROGRESS section, said plainly. Nothing is written to disk: the report and its evidence are printed, and the one temporary collect file is deleted in the same run. `--author` keeps one git author's commits and PRs when a repository has several. The report is plain text with no Markdown, because the document it is pasted into does its own checkboxes and bold; the first real run showed `- [x] ~~…~~` and `**Area**` arriving as literal characters.
