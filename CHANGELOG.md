...
- **The rule about which refusals share a remedy has a gate** — that sentence kept being written as a count, and every count of it drifted. What is stable is the rule, not a number. A refusal about a hook that is present, runnable and does not enforce asks `hook_remedy`; one about a hook that is not runnable, not present, or not this campaign's to displace wants something else and words that itself. The exceptions are asserted by what they are about, so a new one fails a case instead of quietly making the prose false again.
