...
- **A prose pass that corrects before the freeze, inside an envelope** — `review_prose.py` prepares the delta's prose for a reader who has never seen the author's reasoning, and verifies what comes back: every changed line must be a comment, a docstring or markdown, and prose may not grow. Re-reading one's own sentence re-runs the reasoning that wrote it; a reader without that premise checks the sentence against the code. Measured on one campaign: 7 of 27 findings were prose, each costing a full round — freeze, two reviewers, triage, commit — to fix a sentence, while the corrections that answered them wrote 203 lines of prose against 19 of code.
- **Claims a command can settle are settled before the review** — `review_claims.py` refuses a candidate whose prose names something this delta removed from the code, or claims a removal whose quoted subject is still present. A third check — one subject given two counts in a delta — was built and deleted: measured on a real delta it produced seven false positives and no true one, because a docstring narrating history states many numbers about many things. A heuristic at that precision trains a reader to skip the report the exact checks live in. What no command settles is counted and named as unchecked, so silence stops reading as proof. Reviewers now receive the delta with prose hunks elided, the prose:code ratio of what it added, and the scope that follows: a finding whose fix is a sentence belongs to the prose pass and cannot block a receipt.
