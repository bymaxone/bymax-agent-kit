...
- **A custom hooks directory is no longer told to delete its hook** — five refusals offered deletion as the remedy, two of them in wording that does not mention `start` at all. `start` writes only into the repository's own hooks directory and never into a `core.hooksPath` one, so for that reader the remedy was a way to end up with no check at all. One function answers which case the message is in, and the five messages end with its answer.
