...
- **What this repository did not write is derived, not listed** — the prose gate's exclusion list was a hand-kept tuple, and it moved twice losing something each time: first it let the vendored trees be read, then, excluding their parent to keep them out, it took `vendor/README.md` with them — a document this repository wrote about why the folder exists. It is derived now from two markers that exist for their own reasons: `ATTRIBUTION.md`, written beside a vendored tree's `LICENSE` to record where it came from, and the bundler's own `PACKAGE` constant, which names the mirror it generates. A third vendored tree added tomorrow is excluded without anyone remembering to say so.
- **The rule about which refusals share a remedy has a gate** — that sentence had been written wrongly five times: three unchecked counts, then a universal, then a universal with the wrong scope. What is stable is the rule, not a number. A refusal about a hook that exists and does not enforce asks `hook_remedy`; three want a different answer because this one would be wrong for them — a hook that is merely not executable wants `chmod`, a custom hooks directory holding no pre-push wants one added there, and a hook this campaign never installed is never displaced. Those three are now asserted by what they are about, so a fourth exception fails a case instead of quietly making the prose false again.
