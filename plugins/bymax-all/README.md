...
The complete bymax toolkit (after installing the nine siblings above):
...
- 📝 [`bymax-report`](../bymax-report/) — `/bymax-report:standup`, the weekly standup written from evidence: the asks of the period (from your Claude Code and Codex sessions) as a PROGRESS checklist, what shipped (`gh` PRs, `git` commits) as UPDATES by product area, plain text to paste, no Markdown, no files left behind.

## When to use this vs picking individual plugins

- **Just starting** → install all nine. Easier mental model, all tools available.
